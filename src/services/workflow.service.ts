...
export async function individualDonate(
  ...
  const newDonation: IndividualDonation = {
    ...
    donorPhone:    actor.phone || '',   // Captured for requester to contact
...
// ── Admin Rebroadcast: Reset Individual Donor Commitments ──────────────────────
// Used when a committed donor is unreachable/unable to donate.
// Clears all individual donor commitments and re-opens the request for new donors.
export async function rebroadcastRequest(
  requestId: string,
  actor:     UserProfile,
): Promise<void> {
  if (actor.role !== 'admin') {
    throw new Error('Only administrators can rebroadcast a request.');
  }
  ...
  // Must have individual donations to rebroadcast meaningfully
  const individualDonations = request.individualDonations || [];
  if (individualDonations.length === 0) {
    throw new Error('No individual donor commitments to reset on this request.');
  }

  // Calculate how many units were committed by individual donors (to restore)
  const unitsFromIndividualDonors = individualDonations.reduce((sum, d) => sum + d.units, 0);

  // Keep blood bank partial donations intact — only reset individual donors
  const bankFulfilled  = (request.partialDonations || []).reduce((sum, d) => sum + d.units, 0);
  const newUnitsFulfilled = Math.max(0, bankFulfilled);
  const now = Date.now();

  // Reset request — keep bank donations, wipe individual donor commitments
  const updates: Partial<DonationRequest> & Record<string, any> = {
    individualDonations: [],              // Clear all individual commitments
    unitsFulfilled:      newUnitsFulfilled,
    status:              'verified',      // Revert to verified so new donors can step in
    donatedAt:           null,
    updatedAt:           now,
    // Update campName to reflect it's still a broadcast
    campName: `Broadcast — ${request.donorCity || 'District'}`,
  };
  ...
  await set(wfHistRef, {
    ...
    actorName: actor.displayName,
    actorRole: actor.role,
    note:      `REBROADCAST by admin — ${unitsFromIndividualDonors} unit(s) of individual donor commitment(s) cleared. ${individualDonations.map(d => d.donorName).join(', ')} removed. Request reopened for ${request.donorCity}.`,
    timestamp: now,
  });
  ...
  await set(auditRef, {
    id:            auditRef.key,
    type:          'REBROADCAST',
    action:        'ADMIN_REBROADCAST',
    actorUid:      actor.uid,
    actorName:     actor.displayName,
    actorRole:     actor.role,
    targetId:      requestId,
    targetType:    'request',
    previousValue: { individualDonations, status: request.status },
    newValue:      { individualDonations: [], status: 'verified', unitsFulfilled: newUnitsFulfilled },
    metadata:      { referenceNumber: request.referenceNumber },
    timestamp:     now,
  });

  // Notify the requester about the rebroadcast
  try {
    const { sendNotification } = await import('@/services/notification.service');
    const { getProfile }       = await import('@/services/user.service');
    ...
    if (request.createdBy) {
      ...
      if (requester) {
        await sendNotification({
          ...
          recipientPhone: requester.phone,
          recipientName:  requester.displayName,
          title:          `🔄 [BloodBridge] Request Rebroadcast — ${request.referenceNumber}`,
          message:        `Admin has rebroadcast your request (${request.referenceNumber}) for ${request.patientName}. The previous donor commitment has been cleared. A new search for ${request.requiredBloodGroup} donors and blood banks in ${request.donorCity} is now active.`,
          channel:        'both',
          requestId,
          refNumber:      request.referenceNumber,
        });
    ...
    console.error('Rebroadcast notification failed:', notifErr);
  }
}
